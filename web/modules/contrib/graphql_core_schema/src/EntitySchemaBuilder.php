...
class EntitySchemaBuilder {
  ...
  private function getTypeForEntityReferenceFieldItem(FieldItemDataDefinitionInterface $itemDefinition): ?string {
    ...
    // Try to return the GraphQL type of a specific bundle (e.g. TaxonomyTermTag).
    ...
    //   create an interface (e.g. "User" exists only as a type, not an interface).
    ...
    if (
      ...
      // e.g. merge "taxonomy_term" and "tag" to "TaxonomyTermTag".
    ...
    if ($entityType instanceof ConfigEntityTypeInterface) {
      ...
      if ($generatedType && $hasBundles) {
        ...
        foreach (array_keys($bundles) as $bundleId) {
          // Only generate a type if the bundle is enabled.
          if (!$this->config->isBundleEnabled($entityTypeId, $bundleId)) {
            continue;
          }
